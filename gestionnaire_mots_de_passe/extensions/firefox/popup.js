// Gestionnaire de Mots de Passe - Extension Firefox
// Popup principal (compatible WebExtensions)
...
class PasswordManagerPopup {
  ...
  async loadAuthToken() {
    try {
      // Firefox utilise browser.storage au lieu de chrome.storage
      ...
      // Fallback pour anciennes versions de Firefox
      try {
        ...
        console.error('Erreur fallback storage:', fallbackError);
      }
    }
  }
  ...
  async saveAuthToken() {
    if (this.authToken) {
      try {
        await browser.storage.session.set({ authToken: this.authToken });
      } catch (error) {
        // Fallback pour anciennes versions
        await browser.storage.local.set({ authToken: this.authToken });
      }
    }
  }
  ...
  async verifyAuth() {
    try {
      ...
      if (response.ok) {
        ...
        this.authToken = null;
        try {
          await browser.storage.session.remove(['authToken']);
        } catch (error) {
  ...
  async copyGenerated() {
    ...
    if (password) {
      try {
        ...
        this.showSuccess('Mot de passe copié (fallback) !');
      }
    }
  }
...
// Initialiser l'extension (Firefox utilise le namespace browser)
